...
def run(model, trans_softcounts, emission_softcounts, init_softcounts, fixed={}):
    ...
    model["As"][: model["As"].shape[0]] = (
        trans_softcounts
        / np.sum(trans_softcounts, axis=1)[: model["As"].shape[0], None]
    )

    model["learns"] = model["As"][:, 1, 0]
    if "learns" in fixed:
        model["learns"] = model["As"][:, 1, 0] * (fixed["learns"] < 0) + fixed[
            "learns"
        ] * (fixed["learns"] >= 0)
    ...
    if "forgets" in fixed:
        model["forgets"] = model["As"][:, 0, 1] * (fixed["forgets"] < 0) + fixed[
            "forgets"
        ] * (fixed["forgets"] >= 0)
        for i in range(len(model["As"])):
    ...
    if "guesses" in fixed:
        model["guesses"] = model["guesses"] * (fixed["guesses"] < 0) + fixed[
            "guesses"
        ] * (fixed["guesses"] >= 0)

    model["slips"] = model["emissions"][:, 1, 0]
    if "slips" in fixed:
        model["slips"] = model["slips"] * (fixed["slips"] < 0) + fixed["slips"] * (
            fixed["slips"] >= 0
        )
