...
def EM_fit(model, data, tol=0.005, maxiter=100, parallel=True, fixed={}):
    ...
    num_subparts = data["data"].shape[
        0
    ]  # mmm the first dimension of data represents each subpart?? interesting.
    ...
    for i in range(maxiter):
        ...
        for j in range(num_subparts):
            result["all_emission_softcounts"][j] = result["all_emission_softcounts"][
                j
            ].transpose()
...
def run(
    ...
    if "guesses" in fixed:
        guesses = fixed["guesses"] * (fixed["guesses"] < 0) + fixed["guesses"] * (
            fixed["guesses"] >= 0
        )
    if "slips" in fixed:
        slips = fixed["slips"] * (fixed["slips"] < 0) + fixed["slips"] * (
            fixed["slips"] >= 0
        )
    ...
    result["all_trans_softcounts"] = np.reshape(
        all_trans_softcounts, (num_resources, 2, 2), order="C"
    )
    result["all_emission_softcounts"] = np.reshape(
        all_emission_softcounts, (num_subparts, 2, 2), order="C"
    )
    result["all_initial_softcounts"] = all_initial_softcounts
    result["alpha_out"] = alpha_out.flatten(order="F").reshape(
        alpha_out.shape, order="C"
    )
    ...
    for sequence_index in range(sequence_idx_start, sequence_idx_end):
        ...
        for t in range(T - 2, -1, -1):
            ...
            for n in range(num_subparts):
                data_temp = alldata[n][sequence_start + t]
                if data_temp:
                    emission_softcounts_temp[:, (2 * n + int(data_temp == 2))] += gamma[
                        :, t
                    ]
                if f:
