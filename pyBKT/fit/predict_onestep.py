...
def run(model, data):
    ...
    correct_emission_predictions = np.expand_dims(
        model["guesses"], axis=1
    ) @ np.expand_dims(state_predictions[0, :], axis=0) + np.expand_dims(
        1 - model["slips"], axis=1
    ) @ np.expand_dims(
...
def predict_onestep_states(data, model, forward_messages):
    ...
    for sequence_index in range(num_sequences):
        ...
        forward_messages = fd_temp[
            2 * sequence_start : 2 * (sequence_start + T)
        ].reshape((2, T), order="F")
