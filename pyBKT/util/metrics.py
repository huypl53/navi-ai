...
def auc(flat_true_values, pred_values):
    ...
    while i < len(flat_true_values):
        if (flat_true_values[i] != 1 and flat_true_values[i] != 0) or (
            pred_values[i] < 0 or pred_values[i] > 1
        ):
...
def fetch_supported_metrics():
    ...
    for metric_locs in sk._regression, sk._classification:
        potential_metrics = {
            i: getattr(metric_locs, i)
            for i in dir(metric_locs)
            if re.search("_loss$|_score$|_error$", i)
        }
