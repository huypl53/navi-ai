...
def fix_data(data, indices):
    ...
    for i in range(len(prev_starts)):
        ...
        d[:, starts[i] - 1 : starts[i] + lengths[i] - 1] = data["data"][
            :, prev_starts[i] - 1 : prev_starts[i] + lengths[i] - 1
        ]
        resources[starts[i] - 1 : starts[i] + lengths[i] - 1] = data["resources"][
            prev_starts[i] - 1 : prev_starts[i] + lengths[i] - 1
        ]
...
def fix_data_specified(data, label, count):
    ...
    train_data = np.zeros(
        (len(data["data"]), len(data["resources"]) - count), dtype="int"
    )
...
def crossvalidate(model, data, skill, folds, metric, seed, use_folds=False):
    ...
    shuffle = (
        np.random.RandomState(seed=seed) if isinstance(seed, int) else seed
    ).permutation(len(data["starts"]))
    all_true, all_pred = [], []
