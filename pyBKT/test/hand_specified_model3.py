...
observation_sequence_lengths = np.full(
    500, 100, dtype=np.int
...
for r in range(num_learns):
    print(
        ...
        % (
            r + 1,
            truemodel["As"][r, 1, 0].squeeze(),
            best_model["As"][r, 1, 0].squeeze(),
        )
    )
for r in range(num_learns):
    print(
        ...
        % (
            r + 1,
            truemodel["As"][r, 0, 1].squeeze(),
            best_model["As"][r, 0, 1].squeeze(),
        )
    )
...
for s in range(num_gs):
    print(
        "guess%d\t%.4f\t%.4f"
        % (s + 1, truemodel["guesses"][s], best_model["guesses"][s])
    )
for s in range(num_gs):
