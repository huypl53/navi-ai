...
from pydantic import PostgresDsn, RedisDsn, computed_field, MySQLDsn
...
class Settings(BaseSettings):
    ...
    @computed_field
    @property
    def asyncmysql_url(self) -> MySQLDsn:
        """
        This is a computed field that generates a MySQLDsn URL for asyncpg.

        The URL is built using the MultiHostUrl.build method, which takes the following parameters:
        - scheme: The scheme of the URL. In this case, it is "postgresql+asyncpg".
        - username: The username for the Postgres database, retrieved from the MYSQL_USER environment variable.
        - password: The password for the Postgres database, retrieved from the MYSQL_PASSWORD environment variable.
        - host: The host of the Postgres database, retrieved from the MYSQL_HOST environment variable.
        - path: The path of the Postgres database, retrieved from the MYSQL_DB environment variable.

        Returns:
            MySQLDsn: The constructed MySQLDsn URL for asyncpg.
        """
        return MultiHostUrl.build(
